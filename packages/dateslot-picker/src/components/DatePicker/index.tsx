...
const EmptyDate = () => (
  <div
    className="dateslotpicker-w-8 dateslotpicker-h-8 
  dateslotpicker-flex dateslotpicker-items-center dateslotpicker-justify-center"
  ></div>
);
...
const DisableDate = ({
  ...
  ${
    isToday ? 'dateslotpicker-text-green-500' : 'dateslotpicker-text-gray-500'
  }`}
    disabled={true}
  >
    {calendarDate.day}
...
const AvailableDate = ({
  ...
  return (
    <button
      ...
      ${
        isSelected
          ? stylesMap['selectedDate']['dark']
          : isToday
          ? stylesMap['today']['dark']
          : stylesMap['availableDate']['dark']
...
const CalendarDate = (props: CalendarDateType) => {
  ...
  if (
    ...
    return <DisableDate isToday={isToday} calendarDate={calendarDate} />;
  }
...
const DatePicker = (props: IsDateSlotPicker) => {
  const { startDate, endDate, disableWeekly, disableSpecific, disableDate } =
    props;
  const { startDatetime, selectedDate, currentDatetime, onChangeSelectedDate } =
  ...
  return (
    <div
      className={`dateslotpicker-w-[18rem] dateslotpicker-h-[17rem] md:dateslotpicker-h-[18rem] dateslotpicker-rounded-tl-2xl 
    ...
    >
      <div className="md:dateslotpicker-p-5 dateslotpicker-pt-5 dateslotpickerpx-5">
        ...
        >
          <div className="dateslotpicker-font-bold">
            <span
              ...
              className="dateslotpicker-text-sm"
            >
              {monthStr[calendarPeriod.month]}
            </span>
            <span data-testid="calendar-year" className="dateslotpicker-ml-2">
              {calendarPeriod.year}
            </span>
          </div>
        ...
        <div
          className="dateslotpicker-grid dateslotpicker-grid-cols-7 
          dateslotpicker-text-xs dateslotpicker-w-4/5 dateslotpicker-mx-auto"
        >
          {weeklyStr.map((str, index) => (
            <div
              key={index}
              className="dateslotpicker-w-7 dateslotpicker-h-7 
